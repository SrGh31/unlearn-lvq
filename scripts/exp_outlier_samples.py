...
for n in nopts:
    ...
    unlearned_n[cint]={'n':n, 'models': updated_model, 'unlearn_indices': unlearn_indices }
    if (n==nopts[0]) | (cint==0): #'dev_auc_M1M2'
        ...
        cint+=1
        compare_df.to_csv(resultspath+'%s/%s_outlier_unlearn_retrain_compare_nprot%d%s.csv'%(dname, dname,nprots_per_class, solver_type), index=False, sep='\t')
        
        
if cint>0:
    model_sets={'original': glvq, 'retrained': retrained_n, 'unlearned': unlearned_n}
    modelpath='/'.join(parts)+'/models/'
    picklefilename='%s/%s/%s_outlier_samples_nprot%d%s.pkl'%(modelpath, dname, dname, nprots_per_class, solver_type)
