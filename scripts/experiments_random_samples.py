...
#nopts=[0.01,0.05,0.1,1,2]
    ...
    nopts=np.ceil(np.array([0.0001, 0.01, 0.05, 0.2, 0.3])*len(Ytrain)).astype(np.int32)
########################################################################################
...
for n in nopts:
   ...
   # dev02, max_dev_indx02=compare_fidelity_glvq(glvq, glvq_copy)
    ...
    if n==nopts[0]: #'dev_auc_M1M2'
        ...
        compare_df.to_csv(resultspath+'%s_random_unlearn_retrain_compare_nprot%d%s.csv'%(dname,nprots_per_class, solver_type), index=False, sep='\t')

#compare_df.applymap(lambda x: '%.3f' % x)
print('Num prots: ', nprots_per_class)
